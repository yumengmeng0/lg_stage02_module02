...
-- 获取订单编号为order001的订单中的所有商品信息

-- 1.查询订单表项表，获取订单表订单id为order001的数据
SELECT * FROM orderitem WHERE oid = 'order001';

-- 2.将上一步查询的结果作为父查询中的in函数的条件
SELECT * FROM product WHERE pid IN(SELECT pid FROM orderitem WHERE oid = 'order001');
